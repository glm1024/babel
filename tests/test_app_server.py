...
class AppServerSmokeTest(unittest.TestCase):
    ...
    def test_service_scan_state_roundtrip(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn('placeholder="128000"', html)
            ...
            self.assertIn("@media (min-width: 2560px)", html)
            self.assertNotIn('"Iowan Old Style", "Noto Serif SC", serif', html)
            ...
            self.assertIn('poTranslationCurrentReferences.title = ((status.current || {}).references || []).join("\\n");', html)
            self.assertIn("定位字段名", html)
            self.assertIn('placeholder="请输入SQL脚本所在目录绝对路径"', html)
            ...
            self.assertIn('placeholder="请粘贴当前建表 SQL（可选，留空时系统先尝试从当前目录自动推导）"', html)
            self.assertIn('class="sql-schema-label"', html)
            ...
            self.assertGreaterEqual(html.count("const orderedPendingItems = pendingItems.slice().reverse();"), 3)
            ...
            self.assertIn(".sql-schema-input {", html)
            self.assertIn("min-height: 44px;", html)
            self.assertIn(".sql-schema-label {", html)
    ...
    def test_legacy_app_state_migrates_local_default_max_tokens_to_128000(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            out_dir = root / "results"
            out_dir.mkdir()
    ...
    def test_v3_app_state_migrates_local_4096_max_tokens_to_128000(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            out_dir = root / "results"
            out_dir.mkdir()
