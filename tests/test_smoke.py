...
class ScanSmokeTest(unittest.TestCase):
    ...
    def test_service_scan_generates_json_and_report(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("中文硬编码", report)
            self.assertIn("代码注释", report)
            self.assertIn("Swagger注解", report)
            ...
            self.assertNotIn("<th>说明</th>", report)
            ...
            self.assertIn('placeholder="按文本、文件路径或分类搜索"', report)
            self.assertIn("<span>文件路径</span>", report)
            ...
            self.assertIn("当前命中位于 Swagger注解上下文。", report)
            self.assertIn("当前命中用于匹配条件或字符串处理。", report)
            self.assertIn("当前命中位于 ITask注解中。", report)
            ...
            self.assertIn("appearance: none;", report)
            self.assertIn("background-position: right 14px center;", report)
            self.assertIn("pageSize: 10", report)
            self.assertIn('action: "fix"', report)
            self.assertIn('const FIX_REQUIRED_CATEGORY = "FIX_REQUIRED_MERGED";', report)
            self.assertIn('category: FIX_REQUIRED_CATEGORY,', report)
            ...
            self.assertIn('return FIX_REQUIRED_CATEGORY;', report)
            ...
            self.assertIn("function setSingleOption(select, value, group) {", report)
            self.assertIn("function renderCategoryFilterOptions() {", report)
            self.assertIn('return setSingleOption(categoryFilter, FIX_REQUIRED_CATEGORY, "category");', report)
            self.assertIn('state.filters.category = state.filters.action === "fix" ? FIX_REQUIRED_CATEGORY : "";', report)
            ...
            self.assertIn('const headers = ["行号", "项目", "文件路径", "文本", "分类", "动作"];', report)
            ...
            self.assertNotIn('.replaceAll("&", "&amp;")', report)
            self.assertNotIn('.replaceAll("\\\\\\\\", "/")', report)
            self.assertNotIn('.replaceAll(\'"\', \'""\')', report)
            self.assertIn('.replace(/&/g, "&amp;")', report)
            self.assertIn('.replace(/"/g, \'""\')', report)
            self.assertIn('function baseName(path) {', report)
            self.assertIn('const normalized = String(path || "")', report)
            ...
            self.assertIn("flex: 0 1 auto;", report)
            self.assertIn("justify-content: center;", report)
            self.assertIn("text-align: center;", report)
            self.assertIn("vertical-align: middle;", report)
            self.assertIn("margin-left: 0;", report)
            ...
            self.assertIn("--font-ui:", report)
            self.assertIn("--font-mono:", report)
            self.assertIn("@media (min-width: 1920px)", report)
            self.assertIn("@media (min-width: 2560px)", report)
            self.assertNotIn('"Iowan Old Style", "Noto Serif SC", serif', report)
