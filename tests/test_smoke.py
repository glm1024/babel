...
class ScanSmokeTest(unittest.TestCase):
    def test_scan_generates_json_and_report(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn('const label = value === "ALL" ? "全部" : `${value} 条`;', report)
            self.assertIn('state.pageSize = pageSizeSelect.value === "ALL"', report)
            self.assertIn(': Number.parseInt(pageSizeSelect.value, 10) || 10;', report)
